...
const generateTasksJson = (filename) => {
  if (personId) {
  ...
  fs.writeFileSync(filename, JSON.stringify(taskList));
};

const generateTasksText = (filename) => {
  let text = null;
...
const generateTasksCsv = (filename) => {
  let csv = '';
  taskList.forEach(personTask => {
...
generateTasksJson('tasks.json');
generateTasksText('tasks.txt');
generateTasksCsv('tasks.csv');
